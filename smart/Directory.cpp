	...
	const std::string&	dirname)
{
	...
	std::string	pattern = Path::combine(dirname, "*");
	h = FindFirstFile(pattern.c_str(), &fd);
...
#if defined(WIN32)
	const int r_lstat = stat64(s_path, &st);
#else
	const int r_lstat = lstat64(s_path, &st);
#endif
	if (r_lstat  == 0) {
		bool exists;
		exists = (st.st_mode & S_IFDIR) != 0;
#if !defined(WIN32)
		exists = exists && ((st.st_mode & S_IFLNK ) == 0);
#endif
		return exists;
	}
	return false;
...
std::string Directory::verifyAndCreate( std::string root, std::string subdir )
{
	...
	if( subdir.find_first_of(Path::ILLEGAL_CHARACTERS_IN_FILENAME) != std::string::npos )
	{
